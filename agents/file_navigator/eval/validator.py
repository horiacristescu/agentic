...
def get_ground_truth(filesystem: dict) -> dict:
    """Extract all ground truth data from filesystem for validation.

    Args:
        filesystem: Dict representation of filesystem

    Returns:
        Dict with test_files, test_file_sizes, expected_answer, num_test_files
    """

    # Extract all test files
    def extract_files(fs: dict, path: str = "") -> dict[str, int]:
        files = {}
        for name, value in fs.items():
            current_path = f"{path}/{name}" if path else name
            if isinstance(value, dict):
                files.update(extract_files(value, current_path))
            else:
                files[current_path] = value
        return files

    all_files = extract_files(filesystem)
    test_files = {
        path: size
    ...
    return {
        ...
        "expected_answer": sum(test_files.values()),
...
class ToolCallValidator:
    ...
    def __init__(self, filesystem: dict, initial_path: str = "framework"):
        ...
        self.required_calls: list[tuple[str, str]] = [("mocklistdirectory", initial_path)]
...
def validate_trace(
    ...
    # Combine all results
    all_passed = summary["passed"] and answer_check["passed"] and completeness_check["passed"]
