...
from agentic.agents.file_navigator.eval.runner import load_filesystem
from agentic.agents.file_navigator.eval.validator import get_ground_truth, validate_trace
...
def create_successful_trace() -> tuple[list[Message], Result]:
    ...
    messages.append(
        Message(
            ...
            tool_calls=[
                ToolCall(
                    id="call_4a", tool="calculator", args={"operation": "add", "x": 2753, "y": 2269}
                ),
                ToolCall(
                    id="call_4b", tool="calculator", args={"operation": "add", "x": 3923, "y": 7152}
                ),
                ToolCall(
                    id="call_4c", tool="calculator", args={"operation": "add", "x": 9907, "y": 4703}
                ),
                ToolCall(
                    id="call_4d", tool="calculator", args={"operation": "add", "x": 3690, "y": 4574}
                ),
                ToolCall(
                    id="call_4e", tool="calculator", args={"operation": "add", "x": 6288, "y": 3545}
                ),
                ToolCall(
                    id="call_4f", tool="calculator", args={"operation": "add", "x": 4225, "y": 9700}
                ),
                ToolCall(
                    id="call_4g", tool="calculator", args={"operation": "add", "x": 5847, "y": 3255}
                ),
            ],
            timestamp=time.time(),
        )
        ...
        Message(
            ...
            tool_calls=[
                ToolCall(
                    id="call_5a",
                    tool="calculator",
                    args={"operation": "add", "x": 5022, "y": 11075},
                ),
                ToolCall(
                    id="call_5b",
                    tool="calculator",
                    args={"operation": "add", "x": 14610, "y": 8264},
                ),
                ToolCall(
                    id="call_5c",
                    tool="calculator",
                    args={"operation": "add", "x": 9833, "y": 13925},
                ),
        ...
        Message(
            ...
            tool_calls=[
                ToolCall(
                    id="call_6a",
                    tool="calculator",
                    args={"operation": "add", "x": 16097, "y": 22874},
                ),
                ToolCall(
                    id="call_6b",
                    tool="calculator",
                    args={"operation": "add", "x": 23758, "y": 9102},
                ),
            ],
            timestamp=time.time(),
        )
        ...
        Message(
            ...
            tool_calls=[
                ToolCall(
                    id="call_7",
                    tool="calculator",
                    args={"operation": "add", "x": 38971, "y": 32860},
                ),
            ],
            timestamp=time.time(),
        )
...
def test_all_validators_pass_on_successful_trace():
    ...
    # Load filesystem and ground truth
    ...
    # Run validation with new validator
    results = validate_trace(
        messages=messages,
    ...
    assert results["passed"], (
        f"Expected all checks to pass:\n"
        f"  Answer: {results['answer']['passed']}\n"
        f"  Completeness: {results['completeness']['passed']}\n"
        f"  Trace: {results['trace_validation']['passed']}"
    )

    # Verify metrics
    assert results["metrics"]["total_tool_calls"] == 17
    assert results["metrics"]["calculator_calls"] == 13
    assert results["metrics"]["listdir_calls"] == 4
...
def test_validator_catches_hallucinated_value():
    ...
    # Should fail trace validation (hallucinated values)
    assert not results["trace_validation"]["passed"]
    assert len(results["trace_validation"]["violations"]) > 0
...
def test_validator_catches_wrong_answer():
    ...
    # Should fail the answer check
    assert not results["answer"]["passed"]
    assert "wrong_answer" in str(results["answer"]["issues"])
